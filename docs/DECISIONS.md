...
(GCC 15.3, CMake 4.3.1, glibc 2.43, supported until April 2030). It's the lates LTS release and
the included toolchains are very close to those of Ubuntu 26.04, described below.

## Reproducible environments

We want to enable two developers on two different to reproduce the same build.
...
The toolchain itself likes in the `cxx-cmake-container` repository. Every build of the library runs
...
There are some differences though. No Ubuntu build ships CMake 4.3, so we build this from source and
include it. Ubuntu GCC, binutils, and OpenSSL trail by a patch level which is documented in a table
...
shipped with Wyrnose since it's a fairly recent release.
...
It is a simpler choice to build CMake in the container that we ship.
...
- `VERSION` is a `find_package` minimum, not a pin, a container tag and the declared version is the
minimum the code needs. The local lookup is quiet, so read the configure log to learn what resolved.
...
Every source file and every public header have a test file of the same name, which is enforced at
...
A regression can block the release was part of the intended design.
We chose not to perform per-commit benchmarks because gating constantly on minor changes that
inevitably produce deficiencies are noise. It's better to look at the trend over longer periods of time.
